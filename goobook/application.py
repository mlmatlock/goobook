...
from goobook.goobook import Cache
from goobook.goobook import GooBook
from goobook.goobook import GoogleContacts
from goobook.goobook import parse_contacts
from goobook.goobook import parse_groups
...
def main():
    ...
    parser.add_argument(
        "-c", "--config", help="Specify alternative configuration file.", metavar="FILE"
    )
    parser.add_argument(
        "-v",
        "--verbose",
        ...
        action="store_const",
        const=logging.INFO,
        help="Be verbose about what is going on (stderr).",
    )
    parser.add_argument(
        "-V",
        "--version",
    ...
    parser_add.add_argument("name", nargs="?", metavar="NAME", help="Name to use.")
    parser_add.add_argument("email", nargs="?", metavar="EMAIL", help="E-mail to use.")
    parser_add.add_argument(
        "phone", nargs="?", metavar="PHONE", help="Phone number to use."
    )
    parser_add.set_defaults(func=do_add)

    parser_config_template = subparsers.add_parser(
        "config-template", description="Prints a template for .goobookrc to stdout"
    )
    parser_config_template.set_defaults(func=do_config_template)

    parser_dump_contacts = subparsers.add_parser(
        "dump_contacts", description="Dump contacts as JSON."
    )
    parser_dump_contacts.add_argument(
        "-p", "--parse", action="store_true", help="Dump parsed contact instead of raw."
    )
    parser_dump_contacts.set_defaults(func=do_dump_contacts)

    parser_dump_groups = subparsers.add_parser(
        "dump_groups", description="Dump groups as JSON."
    )
    parser_dump_groups.add_argument(
        "-p", "--parse", action="store_true", help="Dump parsed contact instead of raw."
    )
    parser_dump_groups.set_defaults(func=do_dump_groups)

    parser_query = subparsers.add_parser(
        "query", description="Search contacts using query (regex)."
    )
    ...
    parser_reload = subparsers.add_parser(
        "reload", description="Force reload of the cache."
    )
    ...
    parser_auth.add_argument("client_id", metavar="CLIENT_ID", help="Client ID")
    parser_auth.add_argument(
        "client_secret", metavar="CLIENT_SECRET", help="Client secret"
    )
    parser_auth.set_defaults(func=do_authenticate)

    parser_unauth = subparsers.add_parser(
        "unauthenticate", description="Removed authentication data (logout)."
    )
...
def do_dump_contacts(config, args):
    ...
    if args.parse:
        ...
        contacts = unstorageify(
            list(parse_contacts(goco.fetch_contacts(), groupname_by_id))
        )
    ...
    print(
        json.dumps(
            contacts, sort_keys=True, indent=4, ensure_ascii=False, cls=DateEncoder
        )
    )
...
def do_authenticate(config, args):
    ...
    if not creds or creds.invalid:
        flow = oauth2client.client.OAuth2WebServerFlow(
            args.client_id, args.client_secret, SCOPES
        )
