...
from goobook.storage import storageify
...
class GooBook:
    ...
    def query(self, query, simple=False):
        ...
        matching_contacts = sorted(
            self.__query_contacts(query), key=lambda c: c.display_name
        )
        # query groups
        ...
        for title, contacts in matching_groups:
            emails = [
                "%s <%s>" % (c.display_name, c.emails[0][0])
                for c in contacts
                if c.emails
            ]
            emails = ", ".join(emails)
    ...
    def __query_contacts(self, query):
        match = re.compile(
            query.replace(" ", ".*"), re.I
        ).search  # create a match function
        for contact in self.cache.contacts:
            ...
            if any(
                map(
                    match,
                    list(contact.all_names)
                    + [str(number) for (number, kind) in contact.phonenumbers],
                )
            ):
                yield contact
                continue
            matching_addrs = [
                (email, kind) for (email, kind) in contact.emails if match(email)
            ]
            if matching_addrs:
    ...
    def add_mail_contact(self, name, mailaddr, phone=None):
        ...
        log.debug(
            "Going to create contact name: %s email: %s phone: %s",
            name,
            mailaddr,
            phone,
        )
...
class Cache:
    ...
    def load(self, force_update=False):
        ...
        if not force_update and (
            os.path.exists(self.__config.cache_filename)
            and (
    ...
    def save(self):
        ...
        if self.contacts:  # never write a empty addressbook
            cache = {
                "contacts": unstorageify(self.contacts),
...
def parse_contact(person, groupname_by_id):
    ...
    if (
        "birthdays" in person.keys()
        and person["birthdays"]
        and "date" in person["birthdays"][0]
    ):
    ...
    for address in person.get("addresses", []):
        if "formattedValue" in address:
            contact.addresses.append(
                TypedValue(address["formattedValue"], address.get("type", ""))
            )
    ...
    def fetch_contacts(self):
        ...
        request = (
            ...
            .list(
                ...
                sortOrder=None,
                personFields=(
                    "names,nicknames,emailAddresses,memberships,"
